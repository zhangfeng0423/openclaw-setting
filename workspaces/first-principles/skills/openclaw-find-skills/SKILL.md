...
1. `/usr/local/bin/skillhub` (cn-optimized, preferred)
2. `clawhub` (fallback)
...
```bash
/usr/local/bin/skillhub search [query]
```
...
1. Try `/usr/local/bin/skillhub install <slug>` when the result comes from `skillhub`.
